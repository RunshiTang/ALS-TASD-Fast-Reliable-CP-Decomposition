...
result_df = expand.grid(R = R_list, iter = 1:50, sigma = c(0, 0.0001,0.001, 0.01, 0.1,1))
...
result_df <- foreach(i = 1:nrow(result_df), .combine = 'rbind') %dopar% {
  ...
  result_r1als = r1als(random_tensor$tensor + noise_tensor@data * sigma, R, nu = 0.5)
...
write.csv(result_df_save, "result_df_save.csv", row.names = F)
...
result_df = result_df %>% filter(result_df$sigma > 0)
...
plot_list = list()
for (R_tmp in 1:4) {
  p = ggplot(result_df %>% filter(R == R_tmp, type %in% c("TASD-ALS", "CPCA-ICO", 
                                                          "Random ALS", "R1-ALS-1", "R1-ALS-2", 
                                                          "TASD", "SimDiag"))) + 
    geom_boxplot(aes(x = sigma, y = loss, fill = type), outlier.size = 0.5,
                 outlier.alpha = 1, lwd = 0.2) + 
    ggtitle(paste0("R = ", R_tmp)) + 
    xlab("Sigma")+
    labs(fill = "Method")+
    scale_y_log10()+ 
...
ggsave("plot_1.pdf", 
       ggpubr::ggarrange(plotlist = plot_list, ncol = 2, nrow = 2, common.legend = T), 
       width = 10, height = 8)
